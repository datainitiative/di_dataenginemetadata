...
for i in range (1980,datetime.datetime.now().year+1):
...
#	id = models.IntegerField(primary_key=True)
	...
	class Meta:
		db_table = u'dataset_contributor'
		ordering = ['name']
...
class SpatialTable(models.Model):
	...
	id = models.IntegerField(primary_key=True)
	name = models.CharField(max_length=200)
	db_table_name = models.CharField(max_length=50)
	db_table_id = models.IntegerField()
...
# Main Model Tables:
## TableUpload Model
## Add new entry when table is uploaded to database through DB Upload Tool
class TableUpload(models.Model):
#	id = models.IntegerField(primary_key=True)
    ...
    def get_previous_record_id(self):
        ...
        Return the ID of previous record in the table
        '''
        if TableUpload.objects.filter(id__lt=self.id):
            previous_id = TableUpload.objects.filter(id__lt=self.id).order_by('-id')[0].id
        else:
            previous_id = -1
        return previous_id
    ...
    def get_next_record_id(self):
        ...
        Return the ID of previous record in the table
        '''
        if TableUpload.objects.filter(id__gt=self.id):
            next_id = TableUpload.objects.filter(id__gt=self.id).order_by('id')[0].id
        else:
            next_id = -1
        return next_id
...
def post_save_handler_add_tabletags(sender,instance=True,**kwargs):
	...
	try:
		file_name = instance._get_file_name().lower()
		data_table = DataTable.objects.get(db_table=file_name)
		if data_table.table_name != instance.title:
			data_table.table_name = instance.title
			data_table.db_table = file_name
			data_table.save()
		else:
			if data_table.db_table != file_name:
				data_table.db_table = file_name
				data_table.save()
	except:
		data_table = DataTable(id=metadata_id,db_table=file_name,table_name=instance.title)
		data_table.save()	
	...
	table_metadata.metadata = json_metadata
	table_metadata.save()
	table_upload = TableUpload.objects.get(id=instance.id)
	if not table_upload.has_metadata:
		table_upload.has_metadata = True
		table_upload.save()
...
#	subject_matter = models.ForeignKey('SubjectMatter',verbose_name='Subdomain',null=True)
	subject_matter = ChainedForeignKey(
	    SubjectMatter,
	    verbose_name = 'Subdomain',
	    chained_field = 'macro_domain',
	    chained_model_field = 'macro_domain',
	    show_all = False,
	    auto_choose = True
	)	
	...
	if action == 'post_add' and not reverse:
		...
		if len(DatasetMetadata.objects.filter(id=metadata_id)) == 0:
			...
			try:
				...
				json_metadata_dict["contributor"] = instance.contributor.id
